...
def Easter_egg(rectangle2):
    global afficher
    mouse = pygame.mouse.get_pressed()
    if mouse[0]:
        mouse_pos = pygame.mouse.get_pos()
         
        if rectangle2.collidepoint(mouse_pos):
            print('Cliqué sur:', rectangle2)
            print ("hello")
            global R,V,B
...
def menu():
    ...
    boutton2=pygame.draw.rect(screen,(0,0,0),pygame.Rect(0,0,50,50))
    ...
    titre = police_titre.render(("PONG"), False, (R,V,B))
...
def jeu():
    ...
    phrase="".join(lst)
    police = pygame.font.Font("police.ttf", 128)
    texte = police.render((phrase), True, (R, V, B))
    ...
    while chrono<120:
        ...
        texte = police.render((phrase), True, (R, V, B))
        textes = police_chrono.render((chrono2),True,(R,V,B))    
